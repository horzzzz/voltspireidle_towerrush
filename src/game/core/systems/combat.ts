...
import { getTowerAttackSpeed, getTowerDamage, getTowerDeflection } from '../../data/tower-stats';
...
function spawnDamagePopup(world: WorldState, x: number, y: number, amount: number, isBoss: boolean): void {
...
function killEnemy(world: WorldState, enemy: Enemy): void {
  world.charge += enemy.chargeReward;
  world.killCount += 1;
  enemy.hp = 0;
}

function applyDamage(world: WorldState, enemy: Enemy, amount: number): void {
  enemy.hp -= amount;
  spawnDamagePopup(world, enemy.x, enemy.y, amount, enemy.isBoss);
...
export function updateTowerAttack(world: WorldState, dt: number): void {
  ...
  const damage = getTowerDamage(world.tower.levels);
  applyDamage(world, target, damage);
  spawnBolt(world, target.x, target.y);

  const attackSpeed = getTowerAttackSpeed(world.tower.levels);
  world.tower.attackCooldown = 1 / attackSpeed;
}

/** Enemies standing at the tower chip away its HP, reduced by Deflection. */
export function updateContactDamage(world: WorldState, dt: number): void {
  const deflection = getTowerDeflection(world.tower.levels);
  for (const enemy of world.enemies) {
    if (!enemy.inContact || enemy.hp <= 0) continue;
    enemy.attackCooldown -= dt;
    if (enemy.attackCooldown > 0) continue;

    world.tower.health -= enemy.contactDamage * (1 - deflection);
